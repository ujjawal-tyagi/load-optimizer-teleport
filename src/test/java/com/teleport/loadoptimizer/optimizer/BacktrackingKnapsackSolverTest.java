...
class BacktrackingKnapsackSolverTest {

    private KnapsackSolver solver;

    @BeforeEach
    void setUp() {
        solver = new BacktrackingKnapsackSolver();
    }

    @Test
    void solve_shouldProduceSameResultAsBitmaskForPdfExample() {
    ...
    @Test
    void solve_shouldNotExceedWeightLimit() {
        
        OrderInput a = order("a", 100_000L, 25_000, 100);
        OrderInput b = order("b", 200_000L, 25_000, 100);

        Optional<SolverResult> result = solver.solve(List.of(a, b), 30_000, 3_000);

        assertThat(result).isPresent();
        assertThat(result.get().totalWeightLbs()).isLessThanOrEqualTo(30_000);
        
        assertThat(result.get().chosenOrders().get(0).id()).isEqualTo("b");
    }

    @Test
    void solve_shouldNotExceedVolumeLimit() {
        OrderInput a = order("a", 100_000L, 1_000, 1_600);
        OrderInput b = order("b", 100_000L, 1_000, 1_600);

        Optional<SolverResult> result = solver.solve(List.of(a, b), 44_000, 2_000);

        assertThat(result).isPresent();
        assertThat(result.get().totalVolumeCuft()).isLessThanOrEqualTo(2_000);
        assertThat(result.get().chosenOrders()).hasSize(1);
    }

    @Test
    void solve_shouldCompleteForMaximumNEqualsT22() {

        List<OrderInput> orders = java.util.stream.IntStream.rangeClosed(1, 22)
                .mapToObj(i -> order("ord-" + i, 100_000L + i * 1_000L, 1_000, 100))
                .toList();

        Optional<SolverResult> result = solver.solve(orders, 22_000, 2_200);

        assertThat(result).isPresent();
        
        assertThat(result.get().chosenOrders()).hasSize(22);
    }

    @Test
    void solvePareto_shouldReturnNonDominatedFrontier() {
        ...
        assertThat(frontier.get(0).totalPayoutCents()).isEqualTo(500_000L);
    }

    @Test
    void solvePareto_shouldReturnEmptyWhenNothingFits() {
        OrderInput m = order("m", 999_999L, 99_999, 99_999);
        assertThat(solver.solvePareto(List.of(m), 100, 100)).isEmpty();
    }
    ...
    @Test
    void solvePareto_frontierShouldBeSortedByRevenueDescending() {
        OrderInput a = order("a", 300_000L, 30_000, 2_000);
        OrderInput b = order("b", 200_000L, 15_000, 1_000);
        OrderInput c = order("c", 100_000L, 5_000, 300);

        List<SolverResult> frontier = solver.solvePareto(List.of(a, b, c), 44_000, 3_000);

        assertThat(frontier).isNotEmpty();
        
        for (int i = 0; i < frontier.size() - 1; i++) {
            assertThat(frontier.get(i).totalPayoutCents())
                    .isGreaterThanOrEqualTo(frontier.get(i + 1).totalPayoutCents());
        }
    }
